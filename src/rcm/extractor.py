...
class Extractor(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.up2 = up_conv(256, 128)  # 1/8 -> 1/4
        ...
        self.merge_c = nn.Conv1d(256 + 128 + 64, 256, 1)
    ...
    def sparse_branch(self, data):
        ...
        descriptors = self.merge_c(torch.cat([desc1, desc2, desc3], 1))  # 2,4,8,16

        keypoints = torch.stack(keypoints, 0)
        mask0 = ~out_bound_mask(keypoints, data['prepad_size0'])
    ...
    def dense_branch(self, data):
        ...
        desc1 = torch.nn.functional.normalize(self.pool_avg_4(data['d1_1']), p=2, dim=1).reshape(B, -1, h_c*w_c)
        desc2 = torch.nn.functional.normalize(self.pool_avg_2(data['d2_1']), p=2, dim=1).reshape(B, -1, h_c*w_c)
        desc3 = torch.nn.functional.normalize(data['cDa_1'], p=2, dim=1).reshape(B, -1, h_c*w_c)

        # Multi-level Feature
        desc2 = self.lin1(desc2)
        desc1 = self.lin2(desc1)
        descriptors = self.merge_c(torch.cat([desc1, desc2, desc3], 1)).reshape(B, -1, h_c, w_c)  # 2,4,8,16
    ...
    def switch(self, data, switch):
        if 'mask0' in data:
            keys0 = ['image0', 'mask0', 'depth0', 'K0', 'scale0', 'T_0to1', 'feat_f0', 'scores0', 'd1_0', 'd2_0', 'cDa_0', 'prepad_size0']
            keys1 = ['image1', 'mask1', 'depth1', 'K1', 'scale1', 'T_1to0', 'feat_f1', 'scores1', 'd1_1', 'd2_1', 'cDa_1', 'prepad_size1']
        else:
            keys0 = ['image0', 'depth0', 'K0', 'T_0to1', 'feat_f0', 'scores0', 'd1_0', 'd2_0', 'cDa_0', 'prepad_size0']
            keys1 = ['image1', 'depth1', 'K1', 'T_1to0', 'feat_f1', 'scores1', 'd1_1', 'd2_1', 'cDa_1', 'prepad_size1']
        # switch
    ...
    def forward(self, data, mode='test', max_kpts=None):
        ...
        (d1_0, d1_1), (d2_0, d2_1) = d1.split(B), d2.split(B)
